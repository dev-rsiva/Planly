...
const DeleteChecklistCard = ({
  setShowDeleteChecklistCard,
  paramObj,
  workspaceData,
  checklist,
}) => {
  const deleteChecklistCardRef = useRef();
  ...
  const deleteChecklist = (e) => {
    ...
    const generatedObj = (card) => {
      ...
      return {
        ...card,
        checklists: card?.checklists?.filter((eachChecklist) => {
          return eachChecklist?.id !== checklist?.id;
        }),
      };
    };
    ...
    setShowDeleteChecklistCard(false);
  };

  useEffect(() => {
    const handleOutsideClick = (e) => {
      e.stopPropagation();
      if (!deleteChecklistCardRef?.current?.contains(e.target)) {
        setShowDeleteChecklistCard(false);
      }
    };
  ...
  return (
    <>
      <div
        ref={deleteChecklistCardRef}
        onClick={(e) => e.stopPropagation()}
        className="absolute right-[-120px] top-[-20px] bg-white p-4 rounded w-[300px] z-[1801]"
      >
        ...
        <div className="mt-4">
          <p className="font-sans text-sm">
            Deleting an Checklist is permanent and there is no way to get it
            back.
          </p>
        </div>
...
export default DeleteChecklistCard;
