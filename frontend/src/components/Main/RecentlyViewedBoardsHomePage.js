...
import { faX, faStar, faClock } from "@fortawesome/free-regular-svg-icons";
...
const RecentlyViewedBoardsHomePage = () => {
  const navigate = useNavigate();

  const { workspaceData } = useContext(dataContext);
  console.log(workspaceData);

  let recentlyViewedBoards = [];

  workspaceData?.workspaces?.forEach((workspace) => {
    workspace?.boards?.forEach((board) => {
      if (board?.viewedAt !== "" && !board?.starred) {
        recentlyViewedBoards.push({
...
export default RecentlyViewedBoardsHomePage;
